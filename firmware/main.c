...
static int read_result(struct mm_work *mw, struct result *ret)
{
	...
	nonce = test_nonce(mw, ret, longcoinbase);
	if (nonce == NONCE_HW) {
		g_hw_work++;
		return 1;
	}
...
int main(int argv, char **argc)
{
	...
	while (1) {
		...
		if (alink_txbuf_count() < (24 * 5)) {
			...
			miner_gen_nonce2_work(&mm_work, mm_work.nonce2, &work, longcoinbase);
			read_temp0(temperature0);
			get_pkg(&mm_work);
			if (!g_new_stratum)
				continue;
